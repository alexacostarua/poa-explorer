defmodule Indexer.BlockFetcher.Realtime do
  ...
  def new(named_arguments) when is_list(named_arguments) do
    block_fetcher = Keyword.fetch!(named_arguments, :block_fetcher)
    restricted_block_fetcher = %BlockFetcher{block_fetcher | blocks_concurrency: 1}

    interval =
      named_arguments
      |> Keyword.fetch!(:block_interval)
      |> div(2)

    %__MODULE__{
      block_fetcher: restricted_block_fetcher,
      interval: interval
    }
  end
  ...
  def task(%__MODULE__{block_fetcher: %BlockFetcher{json_rpc_named_arguments: json_rpc_named_arguments} = block_fetcher}) do
    ...
    stream_import(%BlockFetcher{block_fetcher | broadcast: false, sequence: sequence})
  end
